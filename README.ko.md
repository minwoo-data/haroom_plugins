...
**해결:** 5명의 특화된 리뷰 에이전트 (conflict detection / improvement / devil's advocate / code review / robustness) 가 타겟을 병렬로 봄. 한 명만 찾은 finding 은 별도 Verifier pass 로 cross-check 해서 false positive 가 시간을 낭비하지 않게 함.
...
**해결:** 3 개의 독립 에이전트가 한 문서를 고정 렌즈 (LLM clarity / architectural longevity / end-user comprehension) 로 라운드를 반복하며 검토. 세 관점이 합의하거나 cap 에 도달할 때까지. 합의는 `updated.md` 에 기록, 원본은 건드리지 않음.
...
**해결:** Claude 가 편집, Codex 가 5개 축 (correctness / security / readability / performance / design) 을 순환하며 비판. 두 모델이 서로의 사각지대를 잡아주며 코드가 수렴할 때까지 반복.
